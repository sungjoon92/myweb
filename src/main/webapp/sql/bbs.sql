...
readcnt , regdt, grpno, indent  : default 값
ip 								: 요청 PC의 IP값 저장
grpno							: max(bbsno) + 1

--그룹번호 만들기
select max(bbsno)  from bt_bbs;
--null값이면 계산 안됨
select max(bbsno) + 1  from bt_bbs;
--nvl() 함수를 이용해서 max(bbsno)값이 비어있을때 null 0으로 바꿈
select nvl(max(bbsno),0) from bt_bbs;
--nvl() 함수를 이용해서 max(bbsno)값이 비어있을때 null 0으로 바꾼 후 + 1
select nvl(max(bbsno),0) + 1  from bt_bbs;
...
from tb_bbs
order by qrpno desc, ansnum asc;