...
public class BbsDAO { //Data Access Object 데이터베이스 관련 작업
    ...
    public int updateproc(BbsDTO dto) { //반환할 값이 없으면 int 대신 void
        int cnt = 0 ;
        try {
           con = dbopen.getConnection();
    ...
    public int reply(BbsDTO dto) { //반환할 값이 없으면 int 대신 void
        int cnt = 0 ;
        try {
    	...
    	try {
			...
			sql.append(search);
			  ...
			  rs= pstmt.executeQuery();    
	           ...
	           if(rs.next()) {
	        	   cnt= rs.getInt("cnt");    //글순서  : 부모글 글순서+1
	           }//if end
	...
	public ArrayList<BbsDTO> list3(String col, String word, int nowPage, int recordPerPage) {
		...
		// 1 패아자 : WHERE r>=1 and f<=10
		// 1 패아자 : WHERE r>=11 and f<=20
		// 1 패아자 : WHERE r>=21 and f<=30
		...
		try {
			...
			if (word.length() == 0) { // 검색어가 존재하지않는경우
